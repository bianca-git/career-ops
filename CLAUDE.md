...
This system was built and used by [santifer](https://santifer.io) to evaluate 740+ job postings, generate 100+ tailored CVs, and land a Head of Applied AI role. The archetypes, scoring logic, negotiation scripts, and proof point structure all reflect his specific career search in AI/automation roles.
...
**THE RULE: When the user asks to customise anything (archetypes, narrative, negotiation scripts, proof points, location policy, comp targets), ALWAYS write to `modes/_profile.md` or `config/profile.yml`. NEVER edit `modes/_shared.md` for user-specific content.** This ensures system updates don't overwrite their customisations.
...
| `analyze-patterns.mjs` | Pattern analysis script (JSON output, filename preserves US spelling) |
...
| `/career-ops-patterns` | `/career-ops patterns` | Analyse rejection patterns and improve targeting |
...
If `modes/_profile.md` is missing, copy from `modes/_profile.template.md` silently. This is the user's customisation file — it will never be overwritten by updates.
...
**After every evaluation, learn.** If the user says "this score is too high, I wouldn't apply here" or "you missed that I have experience in X", update your understanding in `modes/_profile.md`, `config/profile.yml`, or `article-digest.md`. The system should get smarter with every interaction without putting personalisation into system-layer files.
...
This system is designed to be customised by YOU (AI Agent). When the user asks you to change archetypes, translate modes, adjust scoring, add companies, or modify negotiation scripts -- do it directly. You read the same files you use, so you know exactly what to edit.
...
6. Normalise statuses: `node normalize-statuses.mjs` (filename preserves US spelling)
