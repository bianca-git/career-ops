...
| Growth trajectory | 10% | 5=clear path to the next level, 1=dead end |
...
For each offer: score every dimension, then total weighted score.
Final ranking + recommendation with time-to-offer considerations.

Ask the user for the offers if they're not already in context. They can be raw text, URLs, or references to offers already evaluated in the tracker.
