...
5. Detect the company's location → paper size:
   - US/Canada → `letter`
...
8. Pick the top 3-4 projects most relevant to the posting
9. Reorder the experience bullets by relevance to the JD
10. Build the competency grid from the JD's requirements (6-8 keyword phrases)
11. Inject keywords naturally into existing achievements (NEVER invent)
12. Generate the full HTML from the template + tailored content
13. Read `name` from `config/profile.yml` → normalise to lowercase kebab-case (e.g., "John Doe" → "john-doe") → `{candidate}`
...
## Section order (optimised for the "6-second recruiter scan")
...
## Keyword-injection strategy (ethical, grounded in truth)
...
c. Verify the download:
   ...
   ```
   Must show "PDF document". If it shows XML or HTML, the URL has expired — re-export and retry.
...
Update the tracker if the posting is already logged: flip PDF from ❌ to ✅.
